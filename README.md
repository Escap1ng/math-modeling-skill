...
![Stars](https://img.shields.io/github/stars/Escap1ng/math-modeling-skill)
![Forks](https://img.shields.io/github/forks/Escap1ng/math-modeling-skill)
![Issues](https://img.shields.io/github/issues/Escap1ng/math-modeling-skill)
...
- **禁止页眉**：页眉区域必须完全留空，不得出现任何文字、图片或横线（LaTeX 中设置 `\renewcommand{\headrulewidth}{0pt}`）。
- **作者区留空**：`\author{}`、`\date{}` 必须为空，`\maketitle` 不得显示作者与日期。
- **页脚受限**：页脚仅允许居中页码，不得出现学校、队号等身份信息。
- **范围全覆盖**：图片（水印/学校名称）、表格、代码注释、文件名等位置同样不得泄露身份信息。

### 2. 页码规范

- 从**第一页（摘要页）**开始连续编号，摘要页即为第 1 页。
- 页码**页尾居中**显示（`\fancyfoot[C]{\thepage}`）。
- `\maketitle` 后必须添加 `\thispagestyle{fancy}`，覆盖默认 plain 样式，确保摘要页即显示页码。

### 3. 模板与格式同步更新

- LaTeX 模板页眉页脚配置：清空页眉页脚、页尾居中页码、去除页眉横线。
- 排版强制规范新增 **规则 0：匿名与保密要求**。
- Word 文档格式要点：页眉留空、页码页脚居中、匿名要求。

### 4. 质量自检与评分体系强化

- 论文质量自检清单新增 `[致命] 匿名与保密` 检查项（4 项检查及对应修复方法）。
- 评分体系（排版维度）新增：
  - 出现参赛者身份/学校/赛区信息或存在页眉 → **一票否决，直接淘汰，不计分**；
  - 页码未从摘要页（第 1 页）开始，或未页尾居中 → -2。
