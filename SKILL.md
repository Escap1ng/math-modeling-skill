...
**监督学习**：XGBoost、SVM、随机森林
**深度学习**：Transformer、CNN、RNN/LSTM（PINN、GNN见前沿创新方法）
**强化学习**：DQN、PPO

#### 统计与数据分析

**假设检验**：Z/t/卡方检验、Spearman/Kendall相关性
**回归插值**：多元回归、样条插值、响应面法（RSM）、高斯过程回归（GPR）
**降维聚类**：PCA、t-SNE、K-means/DBSCAN
**时间序列**：ARIMA、LSTM/GRU、Prophet

#### 物理与工程建模

**运动动力学**：运动学模型、拉格朗日力学、有限元分析
**仿真计算**：事件驱动仿真、蒙特卡洛仿真、DES
**信号处理**：FFT、小波变换、EMD

#### 前沿创新方法

**数据+物理双驱动**：PINN、算子学习（DeepONet/FNO）
**可解释AI**：符号回归、SHAP/LIME
**多目标优化**：NSGA-II/III、鲁棒优化、贝叶斯优化
**图与网络**：GNN、复杂网络分析
...
| **一、问题重述**        | 背景（引用1-2篇文献）+ 逐问复述                  | 正文第1页                       |
| **二、问题分析**        | 整体思路（建议插入技术路线图）+ 逐问分析               | 正文续                         |
| **三、模型假设**        | 三线表：编号 \| 假设内容 \| 合理性说明（4-6条），表题在**上方** | 正文续                         |
| **四、符号说明**        | 表格：符号 \| 含义 \| 单位                   | 正文续                         |
| **五、模型建立与求解**     | 每问：模型建立 → 求解 → 检验                   | 正文续                         |
| **六、模型的评价、改进与推广** | 6.1 优点 → 6.2 缺点 → 6.3 改进方向 → 6.4 推广 | 正文续                         |
...
（6）模型检验与误差分析
    - 统计检验：残差正态性检验、异方差性检验（如适用）
    - 误差指标：MAE、RMSE、MAPE、R²（至少给出2项）
    - 对比验证：与基准方法/文献结果对比，给出改进百分比
    - 稳健性讨论：简要说明模型对参数扰动的敏感程度
```
...
| 数学公式  | Times New Roman，小四，居中，右侧编号                 |
| 图表标题  | 宋体，五号，居中（图下表上）                             |
| 页码    | 底部居中，从摘要页开始编号（摘要页为第1页）                     |
